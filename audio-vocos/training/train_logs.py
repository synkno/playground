...
class TrainLogs:
    ...
    def log_evaluate(self, global_step, audios):
        ...
        for i, it in  enumerate(audios):
            soundfile.write(f"{out_dir}/{i}-raw.wav", it["wav"].squeeze(0).squeeze(0).detach().cpu().float().numpy(), sr)
            soundfile.write(f"{out_dir}/{i}-gen.wav", it["wav_hat"].squeeze(0).squeeze(0).detach().cpu().float().numpy(), sr)
