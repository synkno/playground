...
class AudioVQ(nn.Module):
    def __init__(self, 
        encoder:Encoder1D, vq:VectorQuantize, decoder:Decoder1D
    ):
        super().__init__()
    ...
    def forward(self, x:torch.Tensor):
        z_e = self.encoder(x)
        vq_out = self.vq(z_e)
        x_rec = self.decoder(vq_out["z_q"])
        vq_out["recons"] = x_rec
        return vq_out
    
def build_model(
    input_dim: int = 128,        # must match EMAVectorQuantize(input_dim)
    codebook_size: int = 256,
    codebook_dim: int = 64,
    decay: float = 0.99,
    reset_threshold: int = 2,
    base_channels: int = 128,
    strides: List[int] = [5, 4, 2, 2],
    num_res_per_stage: int = 1,
    model_file:str = None
):
    vq = VectorQuantize(
