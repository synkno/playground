...
class AttnEncoder(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        for i in range(self.n_layers):
            ...
            self.cross_norm_layers.append(commons.LayerNorm(hidden_channels))
    ...
    def forward(self, x, x_mask, cross_x):#x=32x192x799, x_mask=32x1x799
        ...
        cross_x = cross_x * x_mask
        for i in range(self.n_layers):
            y = self.attn_layers[i](x, x, attn_mask)#y=32x192x799
            y = self.drop(y)
            x = self.norm_layers_1[i](x + y)

            y = self.cross_attn_layers[i](x, cross_x, attn_mask)#y=32x192x799
            y = self.drop(y)
            x = self.cross_norm_layers[i](x + y)
...
class PriorEncoder(nn.Module):
    def __init__(
        ...
        self.attn_enc = AttnEncoder(
            hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout
        )

    def forward(
        self, x, x_mask, f0, spk_emb, noice_scale=1
    ):  # x=6x192x790, x_mask=6x1x790, f0=6x790
        #x = x + self.f0_emb(f0).transpose(1, 2)  # self.f0_emb(f0)=6x790x192
        #cross_x = x + self.f0_emb(f0).transpose(1, 2) + spk_emb
        cross_x = self.f0_emb(f0).transpose(1, 2) + spk_emb
        x = self.attn_enc(x, x_mask, cross_x)  # x=6x192x790
