...
class ResidualCouplingLayer(nn.Module):
    ...
    def forward(self, x, x_mask, g=None, reverse=False):#x=32x192x799, x_mask=32x1x799, g=32x768x1
        ...
        h = self.wav_net(h, x_mask, g=g)#h=32x192x799
        ...
        if not reverse:
            #正向时：输入的是x0, x1 求的是y0，y1
            y1 = m + x1 * torch.exp(logs) * x_mask
            #logs是0，exp(logs)=1, x1=m + x1*x_mask
            y0 = x0
            y = torch.cat([y0, y1], 1)#x=32x192x799
            logdet = torch.sum(logs, [1, 2])#logdet=32x1
            return y, logdet
        else:
            #反向时：输入的是y0, y1 求的是x0，x1, 上面的x0,x1实际上是y0,y1
